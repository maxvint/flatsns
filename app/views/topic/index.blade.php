		...
		<div class="col-xs-12 col-sm-8 col-md-8">
			...
			<div class="topic-list">
				...
				<div id="content">
					...
					<article class="post">
						<div class="avatar">
							<a href="{{ URL::to('user/'.$topic->uid) }}"><img src="{{ asset('img/avatar.jpg') }}" class="img-circle" width="60" height="60" alt=""></a>
						</div>
						<div class="summary">
							<h4><a href="{{ URL::to('topic/show/'.$topic->id) }}">{{	$topic->title }}</a></h4>
							<span class="pull-right badge">{{ $topic->replies }}</span>
							<div class="meta">
								<span><a href="">{{ $topic->user->username }}</a></span> 发表于
								<span class="datetime"><i class="i-time"></i> {{ $topic->create }}</span>
							</div>
						</div>
					</article>
					@endforeach
